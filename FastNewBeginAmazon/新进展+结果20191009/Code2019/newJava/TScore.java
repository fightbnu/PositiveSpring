...
//1) find stressful intervals (SI and U-SI)
//2) output SI/U-SI and neighboring intervals for t-test matlab (ttest.m)
...
public class TScore {
	...
	//Function1***: Find SI and USI
	//Function2***: Calculate t-score
	//FUNCTION: 1)find SI/USI 2) KNN 3)distribution formula
					...
					for(int i =0; i<strAry.size(); i++){
						...
						if(i<strAry.size() && strAry.get(i)>=avgStr){//judge as valid high
							...
							if(i<strAry.size()){
								...
								for(int k=0;k<6;k++){
									cur_upAry.add(0);//accumulated uplift topics (6) 
								}
								for(int k=0; k<5; k++){
									cur_strAry.add(0);//accumulated stressor topics (5) 
								}
								//add
								for(int k=0; k<4; k++){
									cur_postAry.add(0);//accumulated post (4) 
								}
	...
	//Summary: 调用ttestAllTeen(124,K=2)
