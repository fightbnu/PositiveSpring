...
%for T_NUM=1:1:10
    for TOPIC = 0:4
        %In: stress series (intervals only)
        ...
        for pos = 1:length(filelist)
            ...
            if(trainNum>10)
                ...
                %T_NUM = 3;
                    ...
                    M(cx,3) = M(cx,3) + MAPE;
                    M(cx,4) = M(cx,4) + MAD;
                    fprintf(foutMetric,'%s %d %.4f %.4f %.4f %.4f\r\n', filelist(pos).name, T_NUM, MSE, RMSE, MAPE, MAD); %TNUM: predict days
                    % m_path = ['E:\TEST\POSITIVE\Pair\correlation\','metricT',num2str(TOPIC),'124.txt'];
                    % foutMetric = fopen(m_path,'w');
                    % NOTE: T_NUM = 3, 指的是predict 3 days
        ...
        if(USER_ALL>0)%if trainNum>10, USER_ALL++; 
            %add - output average MSE, RMSE, MAPE, MAD for 124 teens
            [curL,corC] = size(M); % M = 10*4
            for xpos=1:1:curL
                fprintf(foutAll,'%d %d %.4f %.4f %.4f %.4f\r\n', TOPIC,xpos, M(xpos,1)/USER_ALL, M(xpos,2)/USER_ALL,...
                M(xpos,3)/USER_ALL, M(xpos,4)/USER_ALL);
                %all_path = 'E:\TEST\POSITIVE\Pair\correlation\metricALL3.txt';
            end
        end
    end %topic end
