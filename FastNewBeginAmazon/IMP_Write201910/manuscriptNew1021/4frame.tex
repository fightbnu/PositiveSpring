...
four measures were adopted: 
\emph{average value of stress, accumulated value of stress, RMS value of stress,} and \emph{maximal value of stress}. 
...
The four measures were denoted as $V_{average}(I)= \frac{1}{n}\sum_{i=1}^{n}(s_i)$, 
$V_{accumulate}(I)= \sum_{i=1}^{n}(s_i)$, 
$V_{RMS}(I) = \sqrt[2]{ \frac{1}{n}\sum_{i=1}^{n}{(s_i)^2}}$, 
$V_{maximal}(I) = \{s_i |\forall s_j \in I \& j \neq i, s_i \geq s_j\}$. 
Similarly, 
we applied the four measures into the positive emotion fluctuations, 
...
where the stress was caused by stressor events in type $e \in E$ and impacted by positive events in type $p \in P$, 
above measures were presented by: 
\begin{equation}
\begin{aligned}
& V_{accumulated}(\textbf{I}_{<e,p>})=\frac{1}{m}\sum_{i=1}^m\frac{1}{n}\sum_{j=1}^n{s_j \in I_i}&
 \end{aligned}
 \end{equation}

For example, in figure \ref{fig:stress} a), 
point (4,1) was the value of average stress in all U-SI intervals,
where stress caused mainly by 'academic' events was impacted by positive events come from 'self-cognition'. 
...
%stressful emotion
The first linguistic measure was the frequency of \emph{stressful emotional words}, 
based on four categories (anger, anxiety, hate, sad) of LICW lexicons ~\citep{Tausczik2010The}, 
represented general stressful emotion during the interval.
%positive emotion
The second measure was the frequency of \emph{positive emotional words}, 
which were identified based on four categories (surprise, joy, expectation, love) of LICW lexicons. 
%stressor topic 
The third measure was the frequency of \emph{stressor event topic words},
representing the degree of attention for five dimensions of stressor events.
Another important factor was wether \emph{self-mentioned words} (i.e., \emph{'I','we','my'}) existed, since self-mentioned words exhibited high probability that the current event was related to the author, 
rather than the opinion about a public event or life events about others (\citep{Li2014Major}). 
For each post, we identified above measures applying the segmentation model and parser model in section \ref{sec:frame1}.
...
Figure \ref{fig:stopic} 
shows the distribution of topic words of stressor events in U-SI and SI, respectively. 
%1) positive不出现使其变深 (应
%2)相应行数变深;但s2,s4共同出现
%3) P2,P3对S1缓解明显 a) b) 
\begin{figure}
