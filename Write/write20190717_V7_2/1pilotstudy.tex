\section{Study1: A pilot study on the stress-buffering function of school scheduled positive events}
\label{sec:study1}
...
\emph{positive event} & 2014/11/5 & \emph{campus art festival} & grade1,2,3\\
\bottomrule
\end{tabular}
}
}
...
There are 122 stressor events and 75 positive events in total.
Here we give the examples of scheduled positive and stressor events in high school life, as shown in Table~\ref{tab:example}.
Comparing the stress curves \emph{a)}, \emph{b)} with \emph{c)},
when an positive event (\emph{campus art festival, holiday} here) happens,
the overall stress intensity during the stressful period is reduced.
An positive event might happen before a teen's stress caused by scheduled stressor events (\emph{example a}),
...
There are 2-3 stressor events and 1-2 positive event scheduled per month.
...
\begin{figure}[H]
\centering
\caption{Examples of school related stressor events, positive events and a student's stress fluctuation}
...
and no other intervention (namely, positive event) occurs.
...
we call the set of such stressful intervals as \textbf{USI}.
...
and 259 academic stressful intervals impacted by four typical scheduled positive events (USI) (in Table \ref{tab:schedule})
